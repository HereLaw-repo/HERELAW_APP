from flask import render_template, request, jsonify, make_response, url_for , session, redirect
import jwt
...
def role_chat():
    ...
    query = f"{what},{answer[1]},{answer[2]},{answer[3]},{answer[4]}"
    ...
    # 협의서 그리기
    image.report_input(query, final_position, final_weather, current_date)

    # 쿼리를 이용해 유사한 비율, 판례 검색
    rate = search.rate_search(query)
    law = search.law_search(query)
...
def chat_chat():
    ...
    for i in range(chat_cnt):
        answer_tokens.append(jwt.decode( request.cookies.get(f"answer_token_{i}"), JWT_SECRET_KEY, algorithms=JWT_ALGORITHMS))
    ...
    if chat_cnt == 2:
        ...
        # 협의서 그리기
        image.report_input(form_result, final_position, final_weather, current_date) #(특이사항칸에 쓰는거, 최종 위치, 최종 날씨)
    ...
    # 사용자에게 줄 엔서 토큰
    return_answer_token = jwt.encode(return_answer_token, JWT_SECRET_KEY, algorithm=JWT_ALGORITHM)
...
def location():
    ...
    # 위치를 이용해 날씨 API 사용
    weather_result = weather.find_location(user_input["latitude"], user_input["longitude"])
...
# result 값 뿌리기 위해 여러개 엔트포인트 만드는 부분 예시
